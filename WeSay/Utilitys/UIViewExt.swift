...
extension UIView {
    func addSubviewExt(view:UIView){
        self.addSubviewExt(view)
    ...
    */
    ...
    class func animation(duration:NSTimeInterval,curve:UIViewAnimationCurve = UIViewAnimationCurve.Linear , delegate:AnyObject? = nil, key:String="key",delay:NSTimeInterval = 0.0 , repeat :Float = 0){
    ...
    class func createByNibNamed(name : String )-> UIView?{
         var array = NSBundle.mainBundle().loadNibNamed(name, owner: nil, options: nil)
        if array.count > 0 {
            return array[0] as? UIView
        }
        return nil
    }
